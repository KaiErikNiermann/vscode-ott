...
const HOM_DESCRIPTIONS: Record<string, string> = {
    ...
    'coq-notation': 'Coq notation — emit a `Notation` rather than a `Definition`',
    'coq-universe': 'Coq universe — the sort (`Set`/`Type`/`Prop`) for this declaration',
    'coq-struct': 'Coq structural recursion — the decreasing argument of a `Fixpoint`',
    'hol': 'HOL proof assistant — generates HOL definitions',
    'isa': 'Isabelle proof assistant — generates Isabelle theory definitions',
    'lem': 'Lem — generates Lem definitions',
    'twf': 'Twelf — generates an LF signature',
